...
const DiscussionCard = ({ item }) => {
  ...
  const handleUpVote = async () => {
    try {
      ...
      const { data } = await axios.post(
        `https://code-clash-server.onrender.com/api/v1/chat/vote/${item._id}`,
        { vote: "up" },
        config
      );
      setUp(data.upvotes);
  ...
  const handleDelete = async () => {
    try {
      ...
      const { data } = await axios.post(
        "https://code-clash-server.onrender.com/api/v1/admin/delete-discussion",
        { chatId: item._id },
        config
      );
      console.log(data);
  ...
  const isBookmark = async () => {
    try {
      ...
      const { data } = await axios.get(
        `https://code-clash-server.onrender.com/api/v1/users/${user.data.user.name}`,
  ...
  const doBookmark = async () => {
    try {
      ...
      const { data } = await axios.post(
        "https://code-clash-server.onrender.com/api/v1/users/add-bookmark",
        { chatId: item._id },
        config
      );
